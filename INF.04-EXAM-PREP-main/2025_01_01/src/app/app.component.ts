...
export class AppComponent {
  checked: boolean[] = [true,true,true];

  ontoggle(category: number) {
    this.checked[category] = !this.checked[category];
  }

  increment_downloads(id: number) {
    this.imgs[this.imgs.findIndex(img => img.id == id)].downloads++;
  }
