# EGZAMIN ZAWODOWY — INF.04 (INF.04-KEYBOARD-SHORTCUTS)
...
Wykonaj aplikację internetową typu front-end obsługującą skróty klawiaturowe dla formularza dodawania elementów z zastosowaniem dostępnego na stanowisku egzaminacyjnym frameworka Angular lub biblioteki React. Zastosuj bibliotekę Bootstrap do zdefiniowania stylu aplikacji.
...
![Obraz 1b](img/keyboard-shortcuts-preview.png)

Na obrazie 1b przedstawiono działanie aplikacji przygotowanej w środowisku React.js, stan po dodaniu elementu za pomocą skrótu klawiaturowego Enter.
W konsoli widoczne jest wyświetlenie komunikatu o użytym skrócie.
...
- Danymi komponentu są: lista elementów, aktualny tekst w polu wejściowym. Dla uproszczenia dane mogą być zdefiniowane jako stan komponentu. Należy założyć, że struktura danych w przyszłości może się zmienić, co będzie miało wpływ na zachowanie i wygląd aplikacji.
- Komponent wyświetla:
  - Nagłówek drugiego stopnia o treści: „Formularz z skrótami klawiaturowymi"
  - Formularz z polem tekstowym do dodawania elementów
  - Przycisk „Dodaj" do ręcznego dodawania elementów
  - Listę dodanych elementów z numeracją
  - Informacje o dostępnych skrótach klawiaturowych
- Aplikacja w stanie początkowym wyświetla puste pole tekstowe i pustą listę elementów
...
- Po naciśnięciu klawisza **Enter** jest generowane zdarzenie, które:
  - Sprawdza czy pole tekstowe nie jest puste (po usunięciu białych znaków)
  - Jeśli dane są poprawne: dodaje element do listy, czyści pole tekstowe, wyświetla w konsoli przeglądarki komunikat w formacie: `"Submit: Enter"`
  - Jeśli dane są niepoprawne: nie wykonuje żadnej akcji
- Po naciśnięciu klawisza **Escape** jest generowane zdarzenie, które:
  - Czyści pole tekstowe
  - Wyświetla w konsoli przeglądarki komunikat w formacie: `"Clear: Escape"`
- Po kliknięciu przycisku „Dodaj" jest generowane zdarzenie, które:
  - Sprawdza czy pole tekstowe nie jest puste
  - Jeśli dane są poprawne: dodaje element do listy, czyści pole tekstowe
- Po dodaniu elementu pole tekstowe automatycznie odzyskuje fokus
...
- Aplikacja web – dowolna liczba zrzutów nazwanych web1, web2 ... (np. stan początkowy, po dodaniu elementu za pomocą Enter, po wyczyszczeniu pola za pomocą Escape, po dodaniu kilku elementów, stan konsoli przeglądarki z wyświetlonymi komunikatami)
