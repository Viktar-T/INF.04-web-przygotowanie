...
function App() {
  ...
  // State for tasks, filters, and history
  ...
  useEffect(() => {
    ...
    const savedHistory = localStorage.getItem('todoAddHistory')
    ...
    if (savedHistory) {
      try {
        ...
        setAddHistory(historyArray)
      } catch (error) {
        console.error('Error loading add history from localStorage:', error)
      }
    }
  }, [])
  ...
  // Save add history to localStorage whenever it changes
  useEffect(() => {
    localStorage.setItem('todoAddHistory', JSON.stringify(addHistory))
  }, [addHistory])
  ...
  const handleAddTask = (event) => {
    ...
    setAddHistory(prev => [...prev, historyRecord])
  ...
  // Clear add history
  const clearAddHistory = () => {
    if (window.confirm('Czy na pewno chcesz wyczyścić historię dodawania zadań?')) {
      setAddHistory([])
      console.log("Historia dodawania zadań wyczyszczona")
    }
  }

  // Get filtered and sorted tasks
  const getFilteredAndSortedTasks = () => {
    const filtered = tasks.filter(task => filters[task.priority])
    
    // Sort by priority: Wysoki → Średni → Niski
  ...
  return (
    <div className="container mt-4">
      <h2 className="mb-4">Lista zadań z priorytetami (Zaawansowana)</h2>
          ...
          {addHistory.length > 0 && (
            <div className="card mb-4">
              <div className="card-header d-flex justify-content-between align-items-center">
                <h5 className="card-title mb-0">Historia dodawania</h5>
                <button
                  className="btn btn-sm btn-warning"
                  onClick={clearAddHistory}
                >
                  Wyczyść historię
                </button>
              </div>
              <div className="card-body">
                <div className="table-responsive">
                  <table className="table table-sm">
                    ...
                    <tbody>
                      {addHistory.map((record, index) => (
                        <tr key={record.id}>
                          <th scope="row">{index + 1}</th>
                          <td>{record.taskText}</td>
                          <td>
            ...
            <div className="card-body">
              ...
              <div className="row text-center">
                <div className="col-6">
                  ...
                  <small className="text-muted">Wszystkie zadania</small>
                </div>
                <div className="col-6">
                  <h5 className="text-primary">{addHistory.length}</h5>
                  <small className="text-muted">Dodane</small>
                </div>
              </div>
            </div>
          </div>
