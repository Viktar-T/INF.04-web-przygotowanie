...
function App() {
  ...
  useEffect(() => {
    const savedHistory = localStorage.getItem('parkingCalculationHistory')
    if (savedHistory) {
      try {
        const history = JSON.parse(savedHistory)
        setCalculationHistory(history)
      } catch (error) {
        console.error('Error loading calculation history from localStorage:', error)
      }
    }
  }, [])

  // Save calculation history to localStorage whenever history changes
  useEffect(() => {
    localStorage.setItem('parkingCalculationHistory', JSON.stringify(calculationHistory))
  }, [calculationHistory])
  ...
  const calculateParkingFee = () => {
    ...
    setCalculationHistory(prev => [...prev, historyRecord])
  ...
  const clearCalculationHistory = () => {
    ...
    console.log("Historia obliczeń wyczyszczona")
  }
          ...
          {calculationHistory.length > 0 && (
            <div className="card mb-4">
              <div className="card-header d-flex justify-content-between align-items-center">
                <h5 className="card-title mb-0">Historia obliczeń</h5>
                <button
                    ...
                    <tbody>
                      {calculationHistory.map((record, index) => (
                        <tr key={record.id}>
                          ...
                          <td>{record.blocks}</td>
                          <td>{record.totalFee.toFixed(2)} PLN</td>
