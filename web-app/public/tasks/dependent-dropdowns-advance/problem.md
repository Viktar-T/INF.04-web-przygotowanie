# EGZAMIN ZAWODOWY — INF.04 (INF.04-DEPENDENT-DROPDOWNS-ADVANCE)
...
Wykonaj aplikację internetową typu front-end obsługującą zależne listy rozwijane z zastosowaniem dostępnego na stanowisku egzaminacyjnym frameworka Angular lub biblioteki React. Zastosuj bibliotekę Bootstrap do zdefiniowania stylu aplikacji.

**+20% OPCJA TRUDNOŚCI:**
- Jeśli kraj nie ma dostępnych miast, wyświetl nieblokujący alert informacyjny i pozwól na przesłanie `{ city: null }` jawnie.
...
![Obraz 1b](img/dependent-dropdowns-preview.png)

Na obrazie 1b przedstawiono działanie aplikacji przygotowanej w środowisku React.js, stan po wybraniu kraju i miasta oraz przesłaniu formularza.
W konsoli widoczne jest wyświetlenie komunikatu z wybranymi wartościami.
...
- Danymi komponentu są: tablica obiektów z krajami i miastami, wybrany kraj, wybrane miasto. Dla uproszczenia dane mogą być zdefiniowane jako stan komponentu. Należy założyć, że struktura danych w przyszłości może się zmienić, co będzie miało wpływ na zachowanie i wygląd aplikacji.
- Komponent wyświetla:
  - Nagłówek drugiego stopnia o treści: „Wybór kraju i miasta (Zaawansowany)"
  - Formularz z dwoma listami rozwijanymi: **Kraj** i **Miasto**
  - Przycisk „Wyślij" do przesłania formularza
  - Informacje o wybranych wartościach
  - **Alert informacyjny gdy kraj nie ma dostępnych miast**
- Aplikacja w stanie początkowym wyświetla puste listy rozwijane z opcją „Wybierz..."
...
- Po wybraniu kraju jest generowane zdarzenie, które:
  - Aktualizuje listę dostępnych miast dla wybranego kraju
  - Resetuje wybrane miasto do wartości pustej
  - Włącza listę miast (jeśli była wyłączona)
  - Wyświetla informację o liczbie dostępnych miast
  - **Jeśli kraj nie ma dostępnych miast, wyświetla alert informacyjny**
- Po wybraniu miasta jest generowane zdarzenie, które:
  - Aktualizuje wybrane miasto
  - Włącza przycisk „Wyślij" (jeśli był wyłączony)
- Po kliknięciu przycisku „Wyślij" jest generowane zdarzenie, które:
  - Sprawdza czy kraj jest wybrany
  - **Jeśli kraj ma miasta: sprawdza czy miasto jest wybrane**
  - **Jeśli kraj nie ma miast: pozwala na przesłanie z `{ city: null }`**
  - Wyświetla w konsoli przeglądarki komunikat w formacie: `{ country: "Nazwa kraju", city: "Nazwa miasta" }` lub `{ country: "Nazwa kraju", city: null }`
  - Wyświetla komunikat o pomyślnym przesłaniu
- Lista miast jest wyłączona do momentu wyboru kraju
- **Jeśli kraj nie ma dostępnych miast, wyświetlany jest nieblokujący alert informacyjny**
- **Przycisk "Wyślij" jest włączony gdy kraj jest wybrany, niezależnie od dostępności miast**
...
- Aplikacja web – dowolna liczba zrzutów nazwanych web1, web2 ... (np. stan początkowy, po wybraniu kraju, po wybraniu miasta, **po wybraniu kraju bez miast**, po przesłaniu formularza, stan konsoli przeglądarki z wyświetlonymi komunikatami)
