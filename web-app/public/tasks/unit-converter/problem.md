# EGZAMIN ZAWODOWY — INF.04 (INF.04-UNIT-CONVERTER)
...
Wykonaj aplikację internetową typu front-end obsługującą konwerter jednostek temperatury z zastosowaniem dostępnego na stanowisku egzaminacyjnym frameworka Angular lub biblioteki React. Zastosuj bibliotekę Bootstrap do zdefiniowania stylu aplikacji.
...
- Danymi komponentu są: wartość w stopniach Celsjusza, wartość w stopniach Fahrenheita, informacja o ostatniej konwersji. Dla uproszczenia dane mogą być zdefiniowane jako stan komponentu. Należy założyć, że struktura danych w przyszłości może się zmienić, co będzie miało wpływ na zachowanie i wygląd aplikacji.
- Komponent wyświetla:
  - Nagłówek drugiego stopnia o treści: „Konwerter temperatury °C ↔ °F"
  - Dwa pola tekstowe: **Celsjusz (°C)** i **Fahrenheit (°F)**
  - Informację o ostatniej konwersji
  - Przykłady konwersji i wzory matematyczne
- Aplikacja w stanie początkowym wyświetla puste pola tekstowe
...
- Po wprowadzeniu wartości w polu Celsjusza jest generowane zdarzenie, które:
  - Sprawdza czy wprowadzona wartość jest liczbą
  - Jeśli wartość jest poprawna: przelicza na Fahrenheita według wzoru: `F = C × 9/5 + 32`
  - Zaokrągla wynik do 1 miejsca po przecinku
  - Wyświetla w konsoli przeglądarki komunikat w formacie: `"C→F: {wartość_C} → {wartość_F}"`
  - Aktualizuje pole Fahrenheita
- Po wprowadzeniu wartości w polu Fahrenheita jest generowane zdarzenie, które:
  - Sprawdza czy wprowadzona wartość jest liczbą
  - Jeśli wartość jest poprawna: przelicza na Celsjusza według wzoru: `C = (F - 32) × 5/9`
  - Zaokrągla wynik do 1 miejsca po przecinku
  - Wyświetla w konsoli przeglądarki komunikat w formacie: `"F→C: {wartość_F} → {wartość_C}"`
  - Aktualizuje pole Celsjusza
- Aplikacja zapobiega pętlom sprzężenia zwrotnego (feedback loops)
- Puste pola są obsługiwane gracjalnie (nie powodują błędów)
...
- Aplikacja web – dowolna liczba zrzutów nazwanych web1, web2 ... (np. stan początkowy, po wprowadzeniu 20°C, po wprowadzeniu 68°F, po wprowadzeniu nieprawidłowej wartości, stan konsoli przeglądarki z wyświetlonymi komunikatami)
...
(Źródło https://getbootstrap.com/docs/4.0/components/input-group/)

Easily extend form controls by adding text, buttons, or button groups on either side of textual inputs, selects, and textareas.
